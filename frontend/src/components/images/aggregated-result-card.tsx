...
function computeWeightedAggregation(images: Image[]): {
	...
	const avgConfidence = winnerConfs.length > 0 ? winnerConfs.reduce((s, x) => s + x, 0) / winnerConfs.length : 0;
...
export function AggregatedResultCard({ consultation, images = null }: AggregatedResultCardProps) {
	const hasImages = images && images.length > 0;
	const computed = hasImages ? computeWeightedAggregation(images) : null;
	const useComputed = computed && computed.condition;

	const condition = useComputed ? computed.condition : consultation.final_predicted_condition;
	const confidence = useComputed ? computed.confidence : (consultation.final_confidence ?? 0);
	...
	return (
		<Card className={cn("border border-slate-200 bg-white", isRefer && "border-amber-200")}>
			<CardHeader className='pb-2'>
				<p className='text-xs font-semibold uppercase tracking-wider text-slate-500'>Aggregated Prediction</p>
			</CardHeader>
			<CardContent className='space-y-3'>
				<div className='flex flex-wrap items-center gap-4'>
					...
					<div>
						<h3 className='font-bold text-slate-900 text-lg'>{formatConditionName(condition)}</h3>
						{totalScans > 0 && (
