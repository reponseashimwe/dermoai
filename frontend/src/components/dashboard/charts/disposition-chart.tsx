...
const COLORS: Record<string, string> = {
	...
	telemedicine_only: "#F59E0B",
	referred_to_clinic: "#3B82F6",
	not_set: "#94A3B8",
};

export function DispositionChart({ data }: DispositionChartProps) {
	const chartData = [
		{ name: "Managed Locally", value: data.treated_locally, key: "treated_locally" },
		{ name: "Referred to Clinic", value: data.referred_to_clinic, key: "referred_to_clinic" },
		{ name: "Telemedicine", value: data.telemedicine_only, key: "telemedicine_only" },
	...
	if (total === 0) {
		return (
			<div className='flex h-64 items-center justify-center text-sm text-slate-500'>No disposition data yet</div>
		);
	}
