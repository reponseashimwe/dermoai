...
export function ConsultationCreateForm() {
	...
	if (isPatient) {
			...
			return (
				<Card>
					<CardHeader>
						...
						<p className='text-sm text-slate-500'>We'll use your account details for this consultation.</p>
					</CardHeader>
					<CardContent className='space-y-4'>
						<div className='rounded-lg border border-slate-200 bg-slate-50 px-4 py-3 text-sm'>
							<p className='font-medium text-slate-700'>{user?.name}</p>
							<p className='text-slate-500'>{user?.email}</p>
							{user?.phone_number && <p className='text-slate-500'>{user.phone_number}</p>}
						</div>
						<Button
		...
		return (
			<Card>
				<CardHeader>
					<h2 className='text-lg font-semibold text-slate-900'>New consultation</h2>
					<p className='text-sm text-slate-500'>Create a consultation for yourself.</p>
				</CardHeader>
				<CardContent className='space-y-4'>
					{myPatient && (
						<Alert variant='info'>
	...
	return (
		<Card>
			<CardHeader>
				<h2 className='text-lg font-semibold text-slate-900'>Select or Create Patient</h2>
